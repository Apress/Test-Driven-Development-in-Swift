...
struct `Nimble Experiments`{
  ...
  func `payment ko shows information alert`() async {
    ...
    await expect(viewModel.shouldShowAlert).toEventually(beTrue())
    await expect(viewModel.alertViewModel?.titleText).toEventually(equal("Payment failed"))
    await expect(viewModel.alertViewModel?.messageText).toEventually(equal("Please contact a waiter."))
    await expect(viewModel.alertViewModel?.dismissButtonText).toEventually(equal("Dismiss"))
  }
}
